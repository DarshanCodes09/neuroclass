...
export default function CreateCourse() {
  ...
  return (
    <div className="p-8 md:p-12 max-w-6xl mx-auto w-full">
      <div className="mb-10">
        <span className="text-xs font-bold tracking-[0.3em] text-indigo-600 uppercase mb-2 block">Instructor</span>
        <h2 className="text-4xl font-headline font-bold tracking-tight text-on-surface dark:text-slate-100">Create Course</h2>
        <p className="text-on-surface-variant dark:text-slate-300 mt-3 text-base max-w-2xl">
          Add the basic course details and upload any rubrics, lecture notes, slides, or references students should be able to open inside the course.
        </p>
      </div>
        ...
        <div className="col-span-12 lg:col-span-8 space-y-12">
          <section className="space-y-6">
            <div className="flex items-center gap-4 mb-2">
              ...
              <h3 className="text-2xl font-headline font-semibold dark:text-slate-100">Course Details</h3>
            </div>
            <div className="bg-surface-container-lowest dark:bg-slate-900/50 p-8 rounded-xl shadow-sm border border-outline-variant/10 dark:border-slate-800 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="col-span-2">
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 mb-2">Course Name</label>
                  <input
                    value={formData.courseName}
                    onChange={(e) => setFormData(prev => ({ ...prev, courseName: e.target.value }))}
                    className="w-full bg-surface-container-low dark:bg-slate-800 border-none rounded-lg p-4 focus:ring-2 ring-primary/20 text-on-surface dark:text-slate-200 placeholder:text-slate-400"
                    placeholder="e.g. Database Management Systems"
                    type="text"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 mb-2">Academic Level</label>
                  <select
                    value={formData.academicLevel}
                    onChange={(e) => setFormData(prev => ({ ...prev, academicLevel: e.target.value }))}
                    className="w-full bg-surface-container-low dark:bg-slate-800 border-none rounded-lg p-4 focus:ring-2 ring-primary/20 text-on-surface dark:text-slate-200">
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 mb-2">Class Capacity</label>
                  <input
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-4 mb-2">
              ...
              <h3 className="text-2xl font-headline font-semibold dark:text-slate-100">Course Files</h3>
            </div>
            ...
            <div
              ...
              className={`border-2 border-dashed border-outline-variant/40 dark:border-slate-700 bg-surface-container-low dark:bg-slate-900/40 p-12 rounded-xl flex flex-col items-center justify-center text-center group transition-colors ${loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-primary/40'}`}>
              ...
              <h4 className="font-headline font-bold text-lg mb-1 dark:text-slate-200">Upload course files</h4>
              <p className="text-on-surface-variant dark:text-slate-400 text-sm max-w-xs">
                Add rubrics, lecture notes, slides, or references that should be available inside the course.
              </p>
            </div>

            {files.length > 0 && (
              <div className="bg-surface-container-lowest dark:bg-slate-900/50 p-6 rounded-xl border border-outline-variant/10 dark:border-slate-800 space-y-3">
                <h4 className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-400 mb-2">Selected Files</h4>
                {files.map((f, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-surface-container-low dark:bg-slate-800 rounded-lg">
                    <div className="flex items-center gap-3">
                      <FileText className="w-5 h-5 text-indigo-500" />
                      <span className="text-sm font-medium dark:text-slate-200">{f.name}</span>
                    </div>
                    {loading ? (
                      <span className="text-xs font-bold text-primary dark:text-indigo-400">{uploadProgress[f.name] || 0}%</span>
                    ) : (
                      <button onClick={() => removeFile(idx)} className="text-on-surface-variant dark:text-slate-400 hover:text-error dark:hover:text-red-400 transition-colors">
                        <X className="w-4 h-4" />
                      </button>
                    )}
                  </div>
